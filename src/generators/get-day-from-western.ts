 ...
 * @return {Day}
 */
const getDayFromWestern = (date: Temporal.PlainDate): Day => {
    const wYear = date.year;
    const jd = date
    ...
    const monthCounts = tibYears.map((y) =>
        monthCountFromTibetan({ year: y, monthCode: 'M01R1' })
    );
    ...
    const { year, monthCode } = getMonthFromMonthCount(monthCount);
